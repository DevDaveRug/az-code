import { PrismaClient, Statut, Source, TypeInteraction } from "@prisma/client";

const prisma = new PrismaClient();

// Seed v0.2 : 4 entreprises + 4 prospects + interactions historisees.
// Aligne sur POST /api/seed. Utilise via `npm run db:seed` en local (necessite DATABASE_URL).
...
async function main() {
  ...
  const legrand = await prisma.entreprise.create({
    data: { nom: "Cabinet Legrand", secteur: "Juridique", taille: "PME", site: "https://legrand.example", adresse: "12 rue de la République, 69001 Lyon", notes: "Cabinet d'avocats spécialisé droit des affaires. 15 collaborateurs." }
  });
  const techflow = await prisma.entreprise.create({
    data: { nom: "TechFlow SAS", secteur: "SaaS B2B", taille: "TPE", site: "https://techflow.example", adresse: "3 rue du Sentier, 75002 Paris", notes: "Éditeur SaaS gestion de projet. Équipe de 8, en croissance." }
  });
  const zenith = await prisma.entreprise.create({
    data: { nom: "Studio Zenith", secteur: "Design", taille: "TPE", site: "https://zenith.example", adresse: "24 rue de Rivoli, 75004 Paris", notes: "Studio design produit et branding. Fondatrice + 3 designers." }
  });
  const marketpro = await prisma.entreprise.create({
    data: { nom: "MarketPro", secteur: "Marketing", taille: "PME", site: "https://marketpro.example", adresse: "8 avenue de la Grande Armée, 75017 Paris", notes: "Agence marketing digital. 20 collaborateurs." }
  });
  ...
  console.log(`Seed v0.2 OK - ${nbEnt} entreprises, ${nbPros} prospects, ${nbInt} interactions`);
}
